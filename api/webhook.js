// Vercel serverless webhook for Stripe payment events (agentdeck)
...
const GH_REPO = process.env.GH_DATA_REPO || 'agentdeck-data';
const MAIL_FROM = process.env.AGENTDECK_MAIL_FROM || 'gentledesk632@agentmail.to';
const AGENTMAIL_KEY = process.env.AGENTMAIL_API_KEY || '';
const BUYERS_FILE = 'buyers.json';
const USERS_FILE = 'users.json';
const ACCESS_PEPPER = process.env.ACCESS_PEPPER || 'agentdeck-pepper-5m2k9t';

function genCode() {
  const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
  let s = '';
  for (let i = 0; i < 10; i++) s += chars[crypto.randomInt(chars.length)];
  return `AD-${s.slice(0, 5)}-${s.slice(5)}`;
}
...
async function sendEmail(buyer, code) {
  ...
  const body =
`Welcome to AgentDeck™ — The AgentDeck™ Launch System ✅

Hi ${buyer.name || 'there'},

Thank you! Your order is confirmed.

YOUR PERSONAL ACCESS CODE: ${code || 'AD-XXXXX-XXXXX'}

OPEN YOUR DOWNLOADS:
→ https://agentdeck.vercel.app/download.html
(enter your email + access code above)

WHAT'S INSIDE (8 deliverables):
• 1. The AgentDeck System — your first 24/7 AI agent in one weekend
• 2. The Job Scorecard — pick the ONE job your agent does
• 3. The Grounding Pack — give your agent its brain (6 must-have files)
• 4. The Prompt Contract — a brain that doesn't drift (+ 20 adversarial tests)
• 5. The Trigger Map — make your agent wake up on its own
• 6. Guardrails & Safeguards — sleep without fear (6 non-negotiables)
• 7. The Receipt Tracker — prove it ran, then improve it
• 8. The AgentDeck Playbook — templates, scripts & your first launch

START TONIGHT:
1. Read The AgentDeck System (10 minutes) — the 4-layer model
2. Score your jobs with the Job Scorecard — pick ONE
3. Build your first draft-mode agent this weekend

60-day, no-questions-asked "Peace of Mind" guarantee — keep everything either way.

The agents are hiring. Be the one who hires them.
— The AgentDeck Team`;
const res = await fetch(`https://api.agentmail.to/v0/inboxes/${MAIL_FROM}/messages/send`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${AGENTMAIL_KEY}` },
    body: JSON.stringify({ to: [buyer.email], subject: "Your AgentDeck™ Launch System access ✅", text: body }),
  });
  return res.ok;
}
